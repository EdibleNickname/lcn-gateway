package com.can.configuration.quqrtz;

import com.can.configuration.quqrtz.model.JobAnno;
import com.can.configuration.quqrtz.model.JobConfig;
import com.can.configuration.quqrtz.model.JobConstant;
	...
	public void startAllJob(){
		...
		try {
			...
			for (JobConfig item : jobConfigSet) {
				...
				//表达式调度构建器
				CronScheduleBuilder scheduleBuilder = CronScheduleBuilder.cronSchedule(item.getCron());

				if (null == trigger) {
					startJob(item);
				} else {
					resetJobCron(item.getJobName(), item.getGroupName(), item.getCron());
				}
	...
	public boolean resetJob(String jobName, String jobGroupName, String cronExpression) {
		...
		for (JobConfig config : jobConfigSet) {
			if(config.getJobName().equals(jobName) && config.getGroupName().equals(jobGroupName)
					&& !config.getCron().equals(cronExpression)) {
